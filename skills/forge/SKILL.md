...
2. `hermes -p <name> skills list` — record the inventory per profile:
   builtins covering the role, generated skills, gaps.
...
- **Never trust a local forge skill of unknown version** — if its frontmatter is not 1.10.0, re-fetch skills/forge/SKILL.md from the repo and follow that copy
...
v0.5.3 context-hygiene update: tune compression and output caps after the approval gate; apply settings per profile; Kanban Desktop is a bundled dashboard plugin.