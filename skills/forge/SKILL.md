...
6. **Single-card-at-a-time (non-negotiable):** the dispatcher's `active_pr` guard is team-wide, not per-bot. If ANY bot has an active PR, ALL queues are held. No new card is dispatched until the active card completes and its PR is merged. To verify: while any bot has an active PR, dispatch a new card — it must be held in `ready` until the active card completes and its PR is merged. Observed live: the `active_pr` guard held the queue ~56 ticks because it was scoped per-bot instead of team-wide.
