...
### Forge v1.30.0
- feat: add single-card-at-a-time serial dispatch rule with verification clause to Forge SKILL.md and workflow-builder SKILL.md (PR #110)
...
- fix: root cause of a live trial — a team was designed around an assumed use case (a founder throwing business ideas) and the coordinator Mission defaulted to all-commercial, so a personal project (Meteoracle, a Meteora LP bot) was killed on commercial viability alone, and a re-submission was killed again on existence. The interview now clarifies the founder's actual goals and goal shape up front so the team and workflow are designed around them, whatever they are.
...
- feat: **reconcile inside, escalate only what is the customer's call** — the Workflow Builder contract must include an internal-reconciliation rule: technical spec/behavior tensions resolve inside the team (the verifier routes to the owning role, e.g. the architect, and adopts that answer unless clearly wrong); no specialist escalates a technical specification question to the customer by default. The customer is the escalation target only for scope/budget trade-offs, external/irreversible actions, or a genuine post-consultation deadlock. Keeps the pipeline autonomous for non-technical users (observed live: a quality-guardian surfaced a log-vs-linear bin-math question to the founder instead of routing it to the architect).
- feat: **founder surface (human interface protocol)** — the generated workflow contract must define how the customer experiences the pipeline, because the board is a bot-to-bot system of record: one short digest per gate (DONE / VERDICT / WAITING ON YOU with explicit reply verbs), one profile (the coordinator) talks to the customer, a **gate-card existence rule** (a routing promise is not complete until the gate card exists — observed live: an R3 push gate was never created, so a founder's approval landed on a dead done card), and customer replies are transcribed verbatim, never executed blind. Workflow Builder v1.6.4 (observed live in trial with a Discord/chat founder surface).
- feat: **mandatory external delivery method** — the generated workflow contract must require that any push to a remote follows the `github-pr-workflow` skill: feature branch off latest `origin/main` (zero-drift check), commit, push the **branch**, open a PR, wait for CI green, then merge (squash) and delete the branch. **Never push directly to `main`/`master`**; a customer "push" approval authorizes the change, not a direct-to-main bypass. Workflow Builder v1.6.5 (observed live: a verified build was pushed straight to `main` because the contract said only "approval required before pushing" without mandating the method).
- No existing rule, template field, historical changelog entry, or procedural content was removed; the change is strictly additive and versioned.
...
- fix: fresh-install acceptance run (free model, sandboxed `HERMES_HOME`) completed a full Team Setup and surfaced four follow-ups: **founder-name neutrality** (agents must label the customer "the founder"; a sandbox agent leaked the host account's first name into every persona and record — add a hard rule and acceptance rows); **per-profile config via CLI** (`hermes config set -p <profile> moa.presets.default.enabled false` + `hermes config get` receipt; the hand-patched top-level `moa.enabled` file shape is not a recognized key); **decision-bot smoke tests must be resolved by the decision-bot profile itself** (no self-composed exchanges or placeholder timestamps); and **`$HERMES_HOME`-relative team-record paths** instead of hardcoded `~/.hermes/TEAM.md` (the run's receipts claimed the default path while the record sat elsewhere).
- fix: corrected the dispatcher claim in `site/llms.txt` — `hermes kanban daemon` is deprecated (the dispatcher now runs in the gateway); agents must verify dispatch/enforcement surface with `hermes kanban dispatch` / `hermes kanban diagnostics` instead of asserting an unverified tick cadence (the sandbox agent reproduced the old claim verbatim).
- fix: extend the version handshake to the **Workflow Builder phase** — observed live in the first supervised trial: the WB phase ran on the stale local cache (`skills/workflow-builder/SKILL.md` v1.4.0 inside `~/.hermes/forge-source/`) while the canonical version on `main` was 1.6.0 — the handshake covered only the forge skill at Team Setup. The manual and both skills now mandate a fresh handshake at the start of **each** phase, treat any local/cached copy as stale-by-default, fetch the canonical skill from `@main`, and record the verdict in receipts.
- fix: **human-source decision gates are never auto-dispatched headless** — observed live in trial 1: the dispatcher auto-claimed the founder go-ahead decision card and spawned the decision bot with no human channel; the spawn exited (`exit_code 1`) and the gate stalled. The manual, both skills, the runbook, the workflow policy, and the acceptance fixture now require: human-source decision cards are created dispatcher-exempt (`blocked`), resolved in the customer's live channel, and a headless spawn of a human gate fails the trial. Also removed the stale `hermes kanban daemon` reference from the Workflow Builder skill (deprecated — the dispatcher runs in the gateway).
- fix: **`sdlc-review` is infrastructure for the review lane, never a disable target** — observed live in trial 1: the run's per-profile skill scoping disabled `sdlc-review` on all six team profiles, and the dispatcher's review lane (which auto-attaches it to `review`-state cards via `kanban.review_dispatch: true`) crashed the founder-gate spawn with `Unknown skill(s): sdlc-review` (exit 1). The Team Setup skill, the Workflow Builder skill, and the acceptance fixture now require keeping `sdlc-review` enabled on every profile that may own `review`-state cards.
- feat: **desktop bot-pool capacity is now a Team Setup optimization** — observed live: a 7-profile team hit the desktop app's pool-slot timeout (*"Too many bots are running at once for this computer's limit"*) and slow bot wakes; the message is a slot-wait timeout (defaults 3 backends / 10 min idle), not a hardware limit. Team Setup's autonomy wiring now raises `maxBackends` (≥ team size + margin) and `idleMs` (≥ 30 min) via Settings → Advanced or the persisted `pool-limits.json` (app-global, applies to all profiles), records the values with the config receipts, and marks headless deploys `SKIPPED`.
- No existing rule, template field, historical changelog entry, or procedural content was removed; the change is strictly additive and versioned.
